...
class RelationsView extends AbstractView implements ViewInterface
{
    public function __construct($contentType, $embedId, $embedInline, $embedSize)
    {
        ...
        if (!$embedObject instanceof \eZContentObject || !$embedObject->canRead()) {
            throw new \Exception(
                \ezpI18n::tr('design/standard/ezoe', 'Invalid parameter: %parameter = %value', null,
                    array('%parameter' => 'EmbedID', '%value' => $embedId))
            );
        }
        ...
        foreach ($imageSizeArray as $size) {
            if ($imageIni->hasVariable($size, 'HideFromRelations')
                && $imageIni->variable($size, 'HideFromRelations') === 'enabled') {
                continue;
            }
            ...
            foreach ($imageIni->variable($size, 'Filters') as $filter) {
                if (strpos($filter, 'geometry/scale') !== false or strpos($filter, 'geometry/crop') !== false) {
                    ...
                    if (isset($filter[1])) {
                        $imagePixelSize = $filter[0] . 'x' . $filter[1];
                    } else {
                        $imagePixelSize = '';
                    }
                }
        ...
        if ($contentIni->hasVariable('embed-inline_' . $embedClassIdentifier, 'AvailableViewModes')) {
            $viewListInline = array_unique($contentIni->variable('embed-inline_' . $embedClassIdentifier,
                'AvailableViewModes'));
        ...
        if ($contentIni->hasVariable('embed-inline_' . $embedClassIdentifier, 'CustomAttributes')) {
            $customAttributes['embed-inline'] = $contentIni->variable('embed-inline_' . $embedClassIdentifier,
                'CustomAttributes');
        } else if ($contentIni->hasVariable('embed-inline-type_' . $contentType, 'CustomAttributes')) {
            $customAttributes['embed-inline'] = $contentIni->variable('embed-inline-type_' . $contentType,
                'CustomAttributes');
        ...
        $tpl->setVariable('attribute_defaults',
            json_encode(array('embed' => $attributeDefaults, 'embed-inline' => $attributeDefaultsInline)));
        ...
        $this->Result = array();
        $content = $tpl->fetch('design:ezoe/tag_embed_' . $contentType . '.tpl');
